...
ds=[findOR([scale(Region(0,0,560,406),Region(0,0,1280,768))],["1652230690072.png","1652324847237.png"]).getTarget()
        ,
findOR([scale(Region(993,494,373,270),Region(0,0,1280,768))],["1652229805622.png","1652323453233.png"]).getTarget()
                ,
findOR([scale(Region(766,0,494,360),Region(0,0,1280,768))],["1652229378603.png","1652322720132.png"]).getTarget()
        ]
...
try:
    ds1=[Region(0,0,313,318).find("1652230690072.png").getTarget(),
...
def my_friend_help_rand():
    if 0==my_click("1655641370783.png"):#click() not work?
        return 0;
    if 0==my_click("1655641922605.png"):#click() not work?
        return 0;
    return 999;
#print my_friend_help_rand();
...
def my_friend_help():
    ...
    try:
        ...
        print "help=",lastLoc;#0.76
        if lastLoc.getScore()<0.88:
            if my_friend_help_rand()==0:
                return 0;
            my_friend_active=False;
            return 999;
        if lastLoc==Location(0,0): 
...
def my_feed1(feedLoc):
    ...
    try:
        loc1=ClientRegion.find(managed("1653360099807.png","my_feed1"));
        loc=loc1.getTarget();
    except:
        pass;
    mouseMove(loc);
...
def my_close_gtx1060():        
    global lastLoc;
    try:
        loc=ClientRegion.find(managed("1654141832902.png","quitgame"))
        type(Key.ESC);
        my.QuitGame=True;
        return 0;
    except:
    ...
    while 1==1:
        ...
        rg=ClientRegion;
        try:   
           rg= Region(1501,39,163,253);           
           loc=rg.find("1640403388181.png");
           Log('Close Alternative Bonus');
           break;
        except:
           pass;        
        try:   
